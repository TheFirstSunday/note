import type { IObj, IPos, LWChartOptions, LWChartTextStyle } from "./utils/interfaces";
...
export default abstract class LWChart<T extends LWChartOptions> {
  constructor (el: HTMLElement, options?: T) {
    ...
    this.createCanvas();
    if(this.options.addTouchEvent) this.addStartEventHandler()
  }
  ...
  private createCanvas () {
    ...
    const n = (i.backingStorePixelRatio ||
  ...
  private startEventHandler (eventType: 'on' | 'off') {
    ...
    const moveEventTypes: (keyof HTMLElementEventMap)[] = isMobile ? ['touchstart'] : ['mouseenter'];
  ...
  private moveEventHandler (eventType: 'on' | 'off') {
    ...
    const moveEventTypes: (keyof HTMLElementEventMap)[] = isMobile ? ['touchmove', 'touchend'] : ['mousemove'];
    const mouseLeaveEventTypes: (keyof HTMLElementEventMap)[] = isMobile ? ['touchend'] : ['mouseleave'];
    moveEventTypes.map(eventType => {
  ...
  protected drawText (text: string, options: LWChartTextStyle) {
    if (!this.ctx) return;
    const { weight, maxWidth } = options;
    let { size, font, color, x, y } = options;
    size = size || 16;
   ...
   */
  protected drawTitle () {
    const { title, titleStyle } = this.options;
  ...
  protected drawCanvasBg () {
    if (!this.ctx) return;
    const { canvasBg = DEFAULT_COLOR } = this.options;
  ...
  protected drawChartBg () {
    if (!this.ctx) return;
    const { chartBg = DEFAULT_COLOR } = this.options;
