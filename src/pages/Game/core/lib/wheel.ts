...
export default class LuckyWheel extends Lucky {
  ...
  protected draw (): void {
    ...
    const start = getAngle(this.rotateDeg - 90 + this.prizeDeg / 2 + _defaultConfig.offsetDegree)
    // 计算文字横坐标
    ...
    this.prizes.forEach((prize, prizeIndex) => {
      // 计算当前奖品区域中间坐标点
      const currMiddleDeg = start + prizeIndex * this.prizeAng
      // 奖品区域可见高度
      ...
      prize.fonts && prize.fonts.forEach(font => {
        ...
        if (wordWrap) {
          lines = splitText(ctx, removeEnter(text), (lines) => {
            ...
            // 最大宽度
            const maxWidth = shortSide * 2 - this.getLength(_defaultConfig.gutter)
    ...
    this.buttons.forEach((btn, btnIndex) => {
      const radius = this.getLength(btn.radius, this.prizeRadius)
      // 绘制背景颜色
      ...
      // 绘制按钮图片
      btn.imgs && btn.imgs.forEach((imgInfo) => {
  ...
  private carveOnGunwaleOfAMovingBoat (): void {
    ...
    while (++i) {
      ...
      const currSpeed = quad.easeOut(this.FPS, stopDeg, endDeg, _defaultConfig.decelerationTime) - stopDeg
      if (currSpeed > speed) {
